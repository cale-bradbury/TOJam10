...
public class CompactDrawer : PropertyDrawer {
  ...
  public override void OnGUI(Rect position, SerializedProperty prop, GUIContent label) {
		...
		switch (prop.type) {
			...
			break;
			case "Vector4":
				var newV4 = EditorGUI.Vector4Field(position, label.text, prop.vector4Value);
				if (EditorGUI.EndChangeCheck()) {
					prop.vector4Value = newV4;
				}
				break;
			case "Quaternionf":
				var newQ = EditorGUI.Vector4Field(position, label.text, QuaternionToVector4(prop.quaternionValue));
				if (EditorGUI.EndChangeCheck()) {
					prop.quaternionValue = ConvertToQuaternion(newQ);
				}
				break;
			default:

				EditorGUI.HelpBox(position, "[Compact] doesn't work with type '" + prop.type + "' (Supported: Vector2, Vector3, Vector4, Quaternion)", MessageType.Error);
				break;
		}
